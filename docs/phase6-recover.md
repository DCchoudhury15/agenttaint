...
the **suggestion is grounded in the AST** — more defensible (and cheaper) than
asking a model to propose a free-form patch.
...
- `fix_suggester/suggest.py`
  ...
  - a parent map assigns each `Call` its true enclosing function (no double
    counting); a call is UNGUARDED iff its enclosing function isn't decorated
    with `@instrument_tool`.
  - `Finding` → `narrate` produces a grounded explanation. The SigNoz MCP read
    (`signoz_search_traces` for similar past violations, to prioritize fixes)
    is a hook for when the MCP server + a model key are configured.
...
- **DLP simulation mode** — `AGENTTAINT_DRY_RUN=1` makes the SDK log the
  ...
  for safe rollout (see what would be flagged before enforcing).
- **RAG** — `rag_retrieve` is already masked at the SDK egress gate (Phase 4:
  rag → `[PII]`/`[SECRET]`, since embeddings are invertible, OWASP LLM08:2025).
  No separate redact-before-embed step is needed because the gate already
  redacts before the tool runs.
...
- README updated with the full 6-phase pipeline + how to run.

## The full arc (detect → propagate → decide → redact → record → recover)

| phase | does | replaces |
|---|---|---|
| 1 | formal DOE model + SigNoz substrate | — |
...
| 5 | tamper-evident Merkle evidence + lineage/SLO dashboards | metrics → verifiable Art. 30/15 records |
